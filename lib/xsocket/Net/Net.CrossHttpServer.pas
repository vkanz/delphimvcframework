...
function TCrossHttpResponse.GetReasonString: string;
begin
...
procedure TCrossHttpResponse.SetReasonString(const Value: string);
begin
  FReasonString := Value;
end;
...
begin
  ...
  if FReasonString.IsEmpty then
    FReasonString := TCrossHttpUtils.GetHttpStatusText(FStatusCode);
...
begin
  ...
  _Send(
    ...
    begin
      ...
      AData^ := @LBuffer[0];
      {TODO -odanielet -cGeneral : Qui LCount sembra essere 0}

      //ACount^ := LBody.Read(LBuffer[0], Min(LCount, SND_BUF_SIZE));
      lPippo := Min(LCount, SND_BUF_SIZE);
      ACount^ := LBody.Read(LBuffer[0], lPippo);
...
procedure TCrossHttpResponse.SendStatus(const AStatusCode: Integer;
  const ACallback: TCrossConnectionCallback);
begin
  SendStatus(AStatusCode, '', ACallback);
end;
